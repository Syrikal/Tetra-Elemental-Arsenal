...
public class EnderUpgrade {
    ...
    public static void teleportCancellation(EntityTeleportEvent event) {
        if (event.getEntity() instanceof LivingEntity livingEntity && livingEntity.hasEffect(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get())) {
            event.setCanceled(true);
            MobEffectInstance dimensional_anchor = livingEntity.getEffect(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get());
    ...
    public static void enderAttack(LivingHurtEvent event) {
        if (!event.getEntity().level().isClientSide) {
            boolean damage_increase = event.getEntity().getType().is(EATags.EntityTypes.END_NATIVE) || event.getEntity().level().dimension().location().getPath().equals("the_end");
    ...
    public static void LootingMobEvent(LootingLevelEvent event) {
        ...
        if (!event.getEntity().level().isClientSide
                ...
                && event.getEntity().getType().is(EATags.EntityTypes.END_NATIVE)) {
    ...
    private static void getAnchoredIdiot(LivingEntity entity) {
        if (entity.hasEffect(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get())) {
            MobEffectInstance anchorInstance = entity.getEffect(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get());
            if (anchorInstance != null) {
                int level = anchorInstance.getAmplifier();
                int new_level = Math.min(level + 1, 3);
                entity.addEffect(new MobEffectInstance(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get(), 200, new_level));
            }
        } else {
            entity.addEffect(new MobEffectInstance(EAMobEffects.DIMENSIONAL_ANCHOR_EFFECT.get(), 200, 0));
        }
    }
