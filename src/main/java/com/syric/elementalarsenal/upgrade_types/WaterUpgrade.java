...
//                    ElementalArsenal.LOGGER.debug("+10% damage due to being in water");
                SendMessageUtil.triggered(UpgradeType.WATER, event.getSource().getEntity());
...
//                ElementalArsenal.LOGGER.debug("Passed ranged checks, increasing damage");
            if (event.getEntity().getType().fireImmune() || event.getEntity().isSensitiveToWater()) {
//                    ElementalArsenal.LOGGER.debug("+20% damage due to target type");
                SendMessageUtil.triggered(UpgradeType.WATER, event.getSource().getEntity());
                event.setAmount(event.getAmount() * 1.2F);
...
//            ElementalArsenal.LOGGER.debug("Detected candidate for water mining speed boost");
...
//            ElementalArsenal.LOGGER.debug("Water shield detected");
...
//                ElementalArsenal.LOGGER.debug("Water shield retaliation activated");
