...
public class LightningUpgrade {
    ...
    public static void lightningAttack(LivingHurtEvent event) {
        if (!event.getEntity().level().isClientSide) {
            ...
            if (shock) {
                event.getEntity().addEffect(new MobEffectInstance(EAMobEffects.SHOCKED_EFFECT.get(), shockDuration, shockLevel - 1));
            }
        }
    }
            ...
            if (!event.getEntity().level().isClientSide()) {
                ...
                blocker.level().getEntities(blocker, search_box).stream()
                        ...
                        .forEach(targetEntity -> {
                            ...
                            if (chargedLevel > 1) {
                                livingTarget.addEffect(new MobEffectInstance(EAMobEffects.SHOCKED_EFFECT.get(), shockDuration, shockedLevel - 1));
                            }
                        });
            }

            blocker.level().playSound(null, blocker.getX(), blocker.getY(), blocker.getZ(), EASounds.LIGHTNING_SHOCKWAVE.get(), SoundSource.PLAYERS, volume, 1.0F);
...
//                            serverLevelData.getThunderTime(), serverLevelData.getRainTime(), serverLevelData.getClearWeatherTime(), serverLevelData.isThundering(), serverLevelData.isRaining());
    ...
    public static MobEffect getChargedEffect() {
        if (ModList.get().isLoaded("irons_spellbooks")) {
            ...
            return EAMobEffects.CHARGED_EFFECT.get();
        }
    }
...
//            ElementalArsenal.LOGGER.debug("Charge level of arrow: " + charge_level);
            return charge_level;
