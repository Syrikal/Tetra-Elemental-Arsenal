...
public abstract class MixinFeatherUtil {
    ...
    public static boolean isFeatherMeleeWeapon(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBladedItem || itemStack.getItem() instanceof ModularDoubleHeadedItem || itemStack.getItem() instanceof ModularSingleHeadedItem) {
            ...
            List<ItemEffect> validEffects = Arrays.asList(Effects.feather_weapon, Effects.feather_both, Effects.ultimate_weapon, Effects.ultimate_both);
    ...
    public static boolean isFeatherRangedWeapon(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBowItem || itemStack.getItem() instanceof ModularCrossbowItem) {
            ModularItem modularItem = (ModularItem) itemStack.getItem();
            EffectData effectData = modularItem.getEffectData(itemStack);
            List<ItemEffect> validEffects = Arrays.asList(Effects.feather, Effects.ultimate);
            return effectData.levelMap.entrySet().stream().anyMatch(entry -> validEffects.contains(entry.getKey()));
        }
    ...
    public static boolean isFeatherTool(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBladedItem || itemStack.getItem() instanceof ModularDoubleHeadedItem || itemStack.getItem() instanceof ModularSingleHeadedItem) {
            ModularItem modularItem = (ModularItem) itemStack.getItem();
            EffectData effectData = modularItem.getEffectData(itemStack);
            List<ItemEffect> validEffects = Arrays.asList(Effects.feather_tool, Effects.feather_both, Effects.ultimate_tool, Effects.ultimate_both);
            return effectData.levelMap.entrySet().stream().anyMatch(entry -> validEffects.contains(entry.getKey()));
        }
