...
public abstract class MixinCorruptUtil {
    ...
    public static boolean isCorruptMeleeWeapon(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBladedItem || itemStack.getItem() instanceof ModularDoubleHeadedItem || itemStack.getItem() instanceof ModularSingleHeadedItem) {
            ...
            List<ItemEffect> validEffects = ModList.get().isLoaded("upgradednetherite_ultimate") ? Arrays.asList(Effects.corrupt_weapon, Effects.corrupt_both, Effects.ultimate_weapon, Effects.ultimate_both) : Arrays.asList(Effects.corrupt_weapon, Effects.corrupt_both);
    ...
    public static boolean isCorruptRangedWeapon(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBowItem || itemStack.getItem() instanceof ModularCrossbowItem) {
            ModularItem modularItem = (ModularItem) itemStack.getItem();
            EffectData effectData = modularItem.getEffectData(itemStack);
            List<ItemEffect> validEffects = ModList.get().isLoaded("upgradednetherite_ultimate") ? Arrays.asList(Effects.corrupt, Effects.ultimate) : Collections.singletonList(Effects.corrupt);
            return effectData.levelMap.entrySet().stream().anyMatch(entry -> validEffects.contains(entry.getKey()));
        }
    ...
    public static boolean isCorruptTool(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ModularBladedItem || itemStack.getItem() instanceof ModularDoubleHeadedItem || itemStack.getItem() instanceof ModularSingleHeadedItem) {
            ModularItem modularItem = (ModularItem) itemStack.getItem();
            EffectData effectData = modularItem.getEffectData(itemStack);
            List<ItemEffect> validEffects = ModList.get().isLoaded("upgradednetherite_ultimate") ? Arrays.asList(Effects.corrupt_tool, Effects.corrupt_both, Effects.ultimate_tool, Effects.ultimate_both) : Arrays.asList(Effects.corrupt_tool, Effects.corrupt_both);
            return effectData.levelMap.entrySet().stream().anyMatch(entry -> validEffects.contains(entry.getKey()));
        }
